...
def participantes_por_ano(ano):
    ...
    # Consulta o total de acertos e erros de cada aluno daquele ano
    cursor.execute("""
        SELECT j.id, j.nome,
               COALESCE(SUM(r.acertou), 0) AS acertos,
               COALESCE(COUNT(r.id) - SUM(r.acertou), 0) AS erros
        FROM jogadores j
        LEFT JOIN resultados r ON j.id = r.jogador_id
        WHERE j.ano = ?
    ...
    lista_jogadores = [
        {"id": j[0], "nome": j[1], "acertos": j[2], "erros": j[3]} 
        for j in jogadores
    ]
