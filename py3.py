...
def handle_novo_jogador(data):
    ...
    # Emite para o cliente o ID do jogador
    emit("jogador_salvo", {"jogador_id": jogador_id})
...
def handle_pegar_jogadores():
    ...
    lista = [{"id": j[0], "nome": j[1], "ano": j[2]} for j in jogadores]

    emit('lista_jogadores', lista)
...
def handle_pegar_relatorio(data):
    ...
    html = "<ul>"
    for r in resultados:
        status = "Acertou" if r[1] else "Errou"
        html += f"<li>{r[0]} - {status} ({r[2]})</li>"
    html += "</ul>"
